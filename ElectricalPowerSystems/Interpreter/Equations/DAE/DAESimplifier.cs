...
namespace ElectricalPowerSystems.Interpreter.Equations.DAE
{
    public partial class DAECompiler
    {
        ...
        ExpressionNode simplifyDerivative(DerivativeNode node)
        {
            if (!variables.ContainsKey(node.Identifier))
            {
                if (parameters.ContainsKey(node.Identifier))
                {
                    compilerErrors.Add(new ErrorMessage("Параметр " + node.Identifier + " используется как дифференциальная переменная", node.Line, node.Position));
                }
            }
            return node;
        }
        ...
        ExpressionNode simplify(ExpressionNode node)
        {
            switch (node.Type)
            {
                ...
                case ASTNodeType.Derivative:
                    return simplifyDerivative((DerivativeNode)node);
            }
            return node;
        }
    }
