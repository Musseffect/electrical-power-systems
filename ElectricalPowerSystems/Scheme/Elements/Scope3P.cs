...
namespace ElectricalPowerSystems.Scheme.Elements
{
    class Scope3P:Element, ITransientElement, ISteadyStateElement, IScopeElement
    {
        ...
        public Scope3P(Pin3Phase in_pin,Pin3Phase out_pin,string label, bool showVoltage, bool showCurrent, bool showPower) :base()
        {
            this.in_pin = in_pin;
        ...
        List<string> IScopeElement.GetTransientVariableNames()
        {
            ...
            if (showVoltage)
                variableNames.AddRange(new string[] { $"{label}.voltageA", $"{label}.voltageB", $"{label}.voltageC" });
            if (showCurrent)
                variableNames.AddRange(new string[] { $"{label}.currentA", $"{label}.currentB", $"{label}.currentC" });
            if (showPower)
                variableNames.AddRange(new string[] { $"{label}.powerA", $"{label}.powerB", $"{label}.powerC",$"{label}.powerAll" });
            return variableNames;
        }
        List<double> IScopeElement.GetReading(TransientState solution)
        {
            List<double> values = new List<double>();
            double va = solution.GetValue(in_pin.VA);
            double vb = solution.GetValue(in_pin.VB);
            double vc = solution.GetValue(in_pin.VC);
            double ia = solution.GetValue(IA);
            double ib = solution.GetValue(IB);
            double ic = solution.GetValue(IC);
            if (showVoltage)
            {
                values.AddRange(new double[] { va, vb, vc});
            }
            if (showCurrent)
            {
                values.AddRange(new double[] { ia, ib, ic });
            }
            if (showPower)
            {
                double pa = va * ia;
                double pb = vb * ib;
                double pc = vc * ic;
                values.AddRange(new double[] { pa, pb, pc, pa+pb+pc });
            }
            return values;
        }
    }
    public class SteadyStateScope3PModel : ISteadyStateElementModel
    {
        ISteadyStateElement ISteadyStateElementModel.CreateElement(Interpreter.Interpreter.Object elementObject, Dictionary<string, Pin> elementNodes)
        {
            ...
            return new Scope3P(elementNodes["in"] as Pin3Phase, elementNodes["out"] as Pin3Phase, label, showVoltage, showCurrent, showPower);
        }
    }
